...
> **Resolved in v3.0.59 — crypto/credential discipline.** When a valid-shaped encrypted blob fails `decrypt()`, `loadCredentialsFromKeychain` now emits a `logger.error` and returns null (fail-closed) rather than serving the coexisting plaintext `connection.password`/`smtpToken` from the same file. Plaintext + failed-auth blob is treated as tamper, not a migration fallback.
