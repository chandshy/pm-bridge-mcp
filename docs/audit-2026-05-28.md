...
> **Resolved in v3.0.57 — permission/grant correctness.** The six email-ID-scoped tools (`delete_email`, `get_email_by_id`, `get_thread`, `mark_email_read`, `star_email`, `download_attachment`) are removed from `FOLDER_AGNOSTIC_TOOLS`, and `extractFolderArg` now reads `sourceFolder`/`source_folder`. A folder-restricted grant enforces the allowlist against the call's `sourceFolder`; a call omitting it fails closed.
